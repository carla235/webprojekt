...
if (!empty($artikelname) && !empty($marke) && !empty($ean) && !empty($preis) && !empty($groesse) && !empty($menge) && !empty($details) && !empty($artikelbeschreibung)) {
    ...
    try {
        ...
        $query = $db->prepare(
            "UPDATE produktkatalog SET marke= :marke, artikelname= :artikelname, artikelnummer= :artikelnummer, artikelbeschreibung= :artikelbeschreibung, ean= :ean, preis= :preis, groesse= :groesse, menge= :menge, details= :details, bild= :bild WHERE artikelnummer = :artikelnummer");         //neuer Inhalt wird reingeschrieben
